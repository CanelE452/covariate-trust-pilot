...
scheme [SBC05; KH06]. Both are functionals of a marginal distribution, and neither
...
not a new subject. Interval autocorrelation, size autocorrelation and dependence between
the two have been varied in simulated demand and shown to change forecast and inventory
performance [ALR12]; estimators have been built that exploit occurrence dependence
rather than assume it away [WSS04]; and neural forecasters have been compared in direct
and decomposed form on simulated series [Kou13]. What has not been isolated is whether
such dependence should change the *representation* a forecaster uses rather than the
estimator applied within it. Prior comparisons of the two representations tune each
separately on a single generated population, rather than holding the budget fixed across
a controlled range of dependence structures.
