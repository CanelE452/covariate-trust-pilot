...
## 2026-08-08 추가 — 세 가설을 통일적으로 설명하는 사실

`docs/rho_definition_synthetic_vs_real.md` (§7-1 확정). 요지:

**descriptor 정의는 문제없다.** 합성 `rho_I` 는 잠재 Markov 상태의 lag-1 자기상관인데,
`gap = d + s` 인 아핀 관계라 관측 gap 수열의 acf 와 **정확히 같은 양**이다
(실측 target ±0.80 -> acf(gap) ∓0.8064/+0.7723). `rho_M` 은 `M = 1+Poisson(λ−1)` 의
수준 내 잡음 때문에 **0.7353 배로 감쇠**한다 (실측 비율 0.70~0.75). 즉 합성 rho_M=+0.8 은
실데이터 관측 척도로 **+0.588** 이다.

**문제는 실데이터가 합성 파라미터 영역에 없다는 것이다.**

```
            |rho_I| p50   max     >=0.80      rho_M p50    >=0.588
m5             0.019     0.646       0개        +0.160        68개
favorita       0.020     0.524       0개        +0.144        14개
```

`|rho_I| >= 0.8` 인 계열이 **양쪽 모두 0개**이고 중앙값은 0.02 다. H2 의
MAG_PERSISTENT cutoff 도 합성 조건의 49%/39% 수준이었다.

-> SCREEN 이 답한 것은 "합성 메커니즘이 실데이터에서 재현되는가" 가 아니라
**"실데이터가 놓인 약한 의존성 영역에서 같은 방향이 관찰되는가"** 다.
H1 이 희소 구간에서 사라지는 것(함정 1)과 H3 가 합성 대비를 못 본 것(함정 2)이
모두 같은 구조적 한계의 다른 얼굴이다.
